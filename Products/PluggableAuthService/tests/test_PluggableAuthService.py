...
class DummyUserEnumerator(DummyPlugin):
    ...
    def enumerateUsers(self, **kw):
        ...
        result = [{'id': _id, 'login': self._login, 'pluginid': self.PLUGINID
                   }]
...
class DummyNotCompetentPlugin(DummyPlugin):
    ...
    def isNotCompetentToAuthenticate(self, request):
        if self.type is None:
            raise KeyError(
                "purposeful `KeyError` by `isNotCompetentToAuthenticate`")
        return self.type
...
class PluggableAuthServiceTests(unittest.TestCase, IUserFolder_conformance, RequestCleaner
    ...
    def test__extractUserIds_authenticate_emergency_user_with_broken_authenticator(self):
        ...
        try:
            ...
            login.extractCredentials = lambda req: {
                'login': '', 'password': ''}
    ...
    def test__verifyUser_login(self):
        ...
        self.failUnless(zcuf._verifyUser(
            zcuf.plugins, login='bar@example.com'))
    ...
    def test__findUser_with_groups_ignoring_one(self):
        ...
        groups = zcuf._getGroupsForPrincipal(
            user, plugins=plugins, ignore_plugins=('bar', ))
        self.assertEqual(len(groups), 2)
    ...
    def test_validate_with_anonymous_factory(self):
        ...
        def _makeAnon():
            user = FauxUser(None, name='New Anonymous User', roles=(), groups={
                            'All People Everywhere Ever': 1})
            return user
    ...
    def test_searchUsers_transform(self):
        ...
        self.failUnless(len(zcuf.searchUsers(
            name=('Foo', 'Bar', 'Zope'))) == 1)
